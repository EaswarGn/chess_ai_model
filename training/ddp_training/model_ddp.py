...
class ChessTemporalTransformerEncoder(nn.Module):
    ...
    def init_weights(self):
        """
        Initializes model weights with recommended strategies for learning.
        """
        ...
        # Initialize special prediction heads
    ...
    def forward(self, batch):
        ...
        from_squares = (self.from_squares(boards[:, 17:, :]).squeeze(2).unsqueeze(1)) if self.from_squares is not None else None
        to_squares = (self.to_squares(boards[:, 17:, :]).squeeze(2).unsqueeze(1)) if self.to_squares is not None else None
