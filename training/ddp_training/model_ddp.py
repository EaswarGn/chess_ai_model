...
class ChessTemporalTransformerEncoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        from_squares = (self.from_squares(boards[:, 14:, :]).squeeze(2).unsqueeze(1)) if self.from_squares is not None else None
        to_squares = (self.to_squares(boards[:, 14:, :]).squeeze(2).unsqueeze(1)) if self.to_squares is not None else None
